 ...
 */
public class SpannedStoreV2 implements CharSequence, GetChars, Spannable, Editable, Appendable , Parcelable {
    /**
     * Create a new SpannableStringBuilder with empty contents
     */
    public SpannedStoreV2() {
        this("");
    }
     ...
     */
    public SpannedStoreV2(CharSequence text) {
     ...
     */
    public SpannedStoreV2(CharSequence text, int start, int end) {
    ...
    public SpannedStoreV2 insert(int where, CharSequence tb, int start, int end) {
        return replace(where, where, tb, start, end);
    }

    // Documentation from interface
    public SpannedStoreV2 insert(int where, CharSequence tb) {
        return replace(where, where, tb, 0, tb.length());
    }

    // Documentation from interface
    public SpannedStoreV2 delete(int start, int end) {
        SpannedStoreV2 ret = replace(start, end, "", 0, 0);
    ...
    // Documentation from interface
    public SpannedStoreV2 append(CharSequence text) {
        int length = length();
        return replace(length, length, text, 0, text.length());
    }

    // Documentation from interface
    public SpannedStoreV2 append(CharSequence text, int start, int end) {
        int length = length();
        return replace(length, length, text, start, end);
    }

    // Documentation from interface
    public SpannedStoreV2 append(char text) {
    ...
    // Documentation from interface
    public SpannedStoreV2 replace(int start, int end, CharSequence tb) {
        return replace(start, end, tb, 0, tb.length());
    }

    // Documentation from interface
    public SpannedStoreV2 replace(final int start, final int end,
                                          CharSequence tb, int tbstart, int tbend) {
    ...
    public static final Parcelable.Creator<SpannedStoreV2> CREATOR  = new Parcelable.Creator<SpannedStoreV2>() {

        public SpannedStoreV2 createFromParcel(Parcel in) {
            return new SpannedStoreV2(in);
        }

        public SpannedStoreV2[] newArray(int size) {
            return new SpannedStoreV2[size];
        }
    };

    private SpannedStoreV2(Parcel in) {
        readFromParcel(in);
    }
    ...
    public void readFromParcel(Parcel in){
        ...
        mSpans = new Object[size];
        for(int index = 0; index < size; index ++){
            mSpans[index] = in.readParcelable(P2ParcelUtils.class.getClassLoader());
        }
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeInt(mGapLength);

        int size = (mSpans != null && mSpans.length > 0 ) ? mSpans.length : -1;
        dest.writeInt(size);
        ...
        dest.writeIntArray(mSpanStarts);
        dest.writeIntArray(mSpanEnds);
